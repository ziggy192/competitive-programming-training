...
public class SerializeAndDeserializeBinaryTree {
	...
	public String serialize(TreeNode root) {
		...
		d.add(new Item(root,1));
		while (!d.isEmpty()){
			Item i = d.removeFirst();
			ensureSize(l,i.p);
			l.set(i.p-1,i.n.val);
			if (i.n.left != null){
				d.addLast(new Item(i.n.left,i.p*2));
			}
			if (i.n.right != null){
				d.addLast(new Item(i.n.right,i.p*2+1));
			}
		}
	...
	private String serialize(ArrayList<Integer> l ){
	...
	private List<Integer> deserialize(String[] st){
		List<Integer> l = new ArrayList<>();
		for (int i =0; i< st.length; i++){
			Integer value;
			if (st[i].equals("null")) value = null;
			else value = Integer.parseInt(st[i]);
			l.add(i,value);
		}
		return l;
	}
	...
	public TreeNode deserialize(String data) {
		...
		List<Integer> l = deserialize(st);
		return dfs(l,0);
	}

	private TreeNode dfs(List<Integer> l, int p){
		if ( p >= l.size() || l.get(p) == null) return null;
		TreeNode node = new TreeNode(l.get(p));
		node.left = dfs(l,2*(p+1)-1);
		node.right = dfs(l,2*(p+1));
		return node;
	}
}
